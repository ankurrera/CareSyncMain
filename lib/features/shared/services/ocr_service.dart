...
class ParsedMedication {
  ...
  @override
  String toString() =>
      '$name | $dosage | $frequency | $duration | Qty: $quantity';
}
...
class OcrService {
  ...
  Future<PrescriptionData> _processPdf(File pdfFile) async {
    ...
    try {
      ...
      tempFile = File('${tempDir.path}/temp_ocr_page_$timestamp.png');
      await tempFile.writeAsBytes(pageImage.bytes);

      await doc.close();
      doc = null;

      final data = await processPrescriptionImage(tempFile);
      return data;
    } catch (e) {
      AppLogger.error(
        'Error processing PDF for OCR',
      ...
      if (tempFile != null && tempFile.existsSync()) {
        try {
          ...
          AppLogger.debug(
            '[OCR] Temp file successfully cleaned up: ${tempFile.path}',
            category: LogCategory.ocr,
          );
        } catch (e) {
          AppLogger.warning(
            '[OCR] Error deleting temp file',
            category: LogCategory.ocr,
            error: e,
          );
        }
...
class PrescriptionTextParser {
  PrescriptionData parse(String text) {
    ...
    for (int i = 0; i < lines.length && i < 10; i++) {
      ...
      if (doctorName == null &&
          RegExp(
            r'\b(Dr\.|Doctor|Dr)\s+([a-zA-Z\s]+)',
            caseSensitive: false,
          ).hasMatch(line)) {
        doctorName =
            line
                .replaceAll(
                  RegExp(r'^(Dr\.|Doctor|Dr)\s*', caseSensitive: false),
                  '',
                )
                .trim();
      }
      if (hospitalName == null &&
          RegExp(
            r'\b(Hospital|Clinic|Medical|Health|Apollo|Care)\b',
            caseSensitive: false,
          ).hasMatch(line)) {
    ...
    final diagnosisRegex = RegExp(
      r'^(Diagnosis|Dx|Impression|Assessment|Provisional Diagnosis)[:\s\-]*',
      caseSensitive: false,
    );
    for (int i = 0; i < lines.length; i++) {
      ...
      if (diagnosisRegex.hasMatch(line)) {
        ...
        bool isSecondaryHeader =
            content.isEmpty ||
            RegExp(
              r'^[/|]\s*Provisional Diagnosis',
              caseSensitive: false,
            ).hasMatch(content);
  ...
  List<ParsedMedication> _extractMedications(List<String> lines) {
    ...
    final abbrFreqPattern = RegExp(
      r'\b(OD|BID|TID|QID|QD|HS|PRN|SOS|STAT|Once\s+a\s+day|Twice\s+a\s+day|Thrice\s+a\s+day)\b',
      caseSensitive: false,
    );

    // 3. Duration: 5 Days, 1 Week, etc.
    final durationPattern = RegExp(
      r'(\d+[-]?\d*)\s*(Days?|Weeks?|Months?|Yrs?|Years?)',
      caseSensitive: false,
    );

    // 4. Strong Medication Indicators (for fallback)
    final medIndicatorPattern = RegExp(
      r'\b(\d+\s*(mg|ml|gm|mcg|unit|u|tablet|cap|syrup|gel|ointment|drops))\b',
      caseSensitive: false,
    );
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (RegExp(
        r'^(Rx|Treatment|Medication|Medicine|Prescribed)',
        caseSensitive: false,
      ).hasMatch(line)) {
        inMedSection = true;
        continue;
      ...
      if (numMatch != null) {
        ...
        if (frequency.contains(' ') &&
            !frequency.contains(RegExp(r'[-.xX*]'))) {
      ...
      if (frequency != null) {
        ...
        String part1 =
            line.substring(0, freqIndex).trim(); // Before freq (Name + Dosage)
        String part2 =
            line
                .substring(freqIndex + frequency.length)
                .trim(); // After freq (Duration + Instructions)
        ...
        int durationDays = 0;
        final durMatch =
            durationPattern.firstMatch(part2) ??
            durationPattern.firstMatch(line);
        if (durMatch != null) {
          duration = durMatch.group(0)!;
          durationDays = _parseDurationToDays(
            durMatch.group(1)!,
            durMatch.group(2)!,
          );
        }
    ...
    if (freqUpper.contains('OD') || freqUpper.contains('ONCE')) {
      ...
      for (var p in parts) {
        if (p.contains('/')) {
          ...
          if (frac.length == 2) {
            dailyDose +=
                (double.tryParse(frac[0]) ?? 0) /
                (double.tryParse(frac[1]) ?? 1);
          }
        } else {
  ...
  bool _isFooterOrJunk(String line) {
    final junk = RegExp(
      r'\b(Page|Signature|Sign|Reg No|Reg\.No|Address|Phone|Email|Electronic Signature|Date|Dated)\b',
      caseSensitive: false,
    );
